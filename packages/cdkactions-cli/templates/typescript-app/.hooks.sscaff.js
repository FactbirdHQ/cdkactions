...
exports.post = (ctx) => {
  ...
  if (!npm_cdkactions) {
    throw new Error(`missing context "npm_cdkactions"`);
  }
  if (!npm_cdkactions_cli) {
    throw new Error(`missing context "npm_cdkactions_cli"`);
  }

  installDeps([npm_cdkactions, `constructs@${constructs_version}`]);
  installDeps([npm_cdkactions_cli, '@types/node', 'typescript'], true);
